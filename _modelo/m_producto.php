...
//-----------------------------------------------------------------------
// FUNCIONES PARA MODAL DE PRODUCTOS CON STOCK POR TIPO DE PRODUCTO
//-----------------------------------------------------------------------

function NumeroRegistrosTotalProductosConStockPorTipo($id_almacen, $id_ubicacion, $tipoProducto = 0) {
    include("../_conexion/conexion.php");
    
    $whereClause = "WHERE p.est_producto = 1";
    
    if ($tipoProducto > 0) {
        $whereClause .= " AND p.id_producto_tipo = $tipoProducto";
    }
...
function NumeroRegistrosFiltradosProductosConStockPorTipo($search, $id_almacen, $id_ubicacion, $tipoProducto = 0) {
    include("../_conexion/conexion.php");
    mysqli_set_charset($con, "utf8");
...
function MostrarProductosConStockPorTipo($limit, $offset, $search, $orderColumn, $orderDirection, $id_almacen, $id_ubicacion, $tipoProducto = 0) {
    include("../_conexion/conexion.php");
...
function MostrarProductosModalFiltrado($limit, $offset, $searchValue, $orderColumn, $orderDirection, $id_almacen, $id_ubicacion, $tipoProducto = 0)
{
    ...
    $whereConditions = "p.est_producto = 1 
    ...
    if ($tipoProducto > 0) {
        $whereConditions .= " AND p.id_producto_tipo = $tipoProducto";
    }

    // Aplicar filtro de búsqueda ANTES del GROUP BY
    if (!empty($searchValue)) {
        $searchValue = mysqli_real_escape_string($con, $searchValue);
        $whereConditions .= " AND (p.cod_material LIKE '%$searchValue%' 
    ...
    $sql = "SELECT p.id_producto, p.cod_material, p.nom_producto, p.mar_producto, p.mod_producto,
                   pt.nom_producto_tipo, um.nom_unidad_medida, um.id_unidad_medida,
                   COALESCE(SUM(
                    CASE
                        -- INGRESOS
            ...
            GROUP BY p.id_producto, p.cod_material, p.nom_producto, p.mar_producto, p.mod_producto, 
                     pt.nom_producto_tipo, um.nom_unidad_medida, um.id_unidad_medida
