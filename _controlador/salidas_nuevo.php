    ...
    <div class="container body">
        <div class="main_container">
            ...
            // Variables para mostrar alertas
                ...
                //=======================================================================
                // PROCESAR FORMULARIO CON CENTROS DE COSTO
                //=======================================================================
                if (isset($_REQUEST['registrar'])) {
                    ...
                    //  CAMBIO: ndoc_salida siempre vacío (se guardará NULL en la BD)
                    $ndoc_salida = '';
                    ...
                    if ($id_material_tipo == 1) {
                        GrabarAuditoria($id, $usuario_sesion, 'ERROR AL REGISTRAR', 'SALIDAS', "Tipo material inválido (NA)");
                    ...
                    // Obtener nombres para auditoría
                        ...
                        GrabarAuditoria($id, $usuario_sesion, 'ERROR AL REGISTRAR', 'SALIDAS', 
                            "Ubicación origen = destino: $almacen_nombre - $ubicacion_nombre");
                        ...
                        //  PROCESAR MATERIALES CON CENTROS DE COSTO
                        $materiales = array();
                            ...
                            foreach ($_REQUEST['id_producto'] as $index => $id_producto) {
                                if (!empty($id_producto) && !empty($_REQUEST['cantidad'][$index])) {
                                    ...
                                    if ($stock_disponible <= 0) {
                                        GrabarAuditoria($id, $usuario_sesion, 'ERROR AL REGISTRAR', 'SALIDAS', 
                                            "Stock insuficiente: {$_REQUEST['descripcion'][$index]}");
                                        ...
                                        GrabarAuditoria($id, $usuario_sesion, 'ERROR AL REGISTRAR', 'SALIDAS', 
                                            "Cantidad excede stock: {$_REQUEST['descripcion'][$index]} (Sol: $cantidad, Disp: $stock_disponible)");
                                        ...
                                        //  CAPTURAR CENTROS DE COSTO
                                        $centros_costo_material = [];
                                        if (isset($_REQUEST['centros_costo'][$index]) && is_array($_REQUEST['centros_costo'][$index])) {
                                            // Filtrar valores vacíos y convertir a enteros
                                            $centros_costo_material = array_filter(
                                                array_map('intval', $_REQUEST['centros_costo'][$index]),
                                                function($val) { return $val > 0; }
                                            );
                                        }
                                        
                                        //  VALIDACIÓN 4: Verificar que tenga centros de costo (SOLO si NO viene de pedido)
                                        if (empty($centros_costo_material) && $id_pedido_inicial == 0) {
                                            GrabarAuditoria($id, $usuario_sesion, 'ERROR AL REGISTRAR', 'SALIDAS', 
                                                "Sin centros de costo: {$_REQUEST['descripcion'][$index]}");
                                            
                                            $mostrar_alerta = true;
                                            $tipo_alerta = 'warning';
                                            $titulo_alerta = 'Centros de costo requeridos';
                                            $mensaje_alerta = "Debe seleccionar al menos un centro de costo para el producto '{$_REQUEST['descripcion'][$index]}'.";
                                            break;
                                        }
                                        
                                        //  AGREGAR MATERIAL CON CENTROS DE COSTO
                                        $materiales[] = array(
                                            'id_producto' => intval($id_producto),
                                            'descripcion' => $_REQUEST['descripcion'][$index],
                                            'cantidad' => $cantidad,
                                            'centros_costo' => $centros_costo_material  // 🔹 AGREGADO
                                        );
                                    }
                                }
                            }
                        ...
                        // VALIDACIÓN 5: Verificar que haya al menos un material
                        if (!$mostrar_alerta && count($materiales) == 0) {
                            GrabarAuditoria($id, $usuario_sesion, 'ERROR AL REGISTRAR', 'SALIDAS', "Sin materiales");
                        ...
                        if (!$mostrar_alerta && count($materiales) > 0) {
                            ...
                            if ($resultado === "SI" || (is_array($resultado) && isset($resultado['success']) && $resultado['success'] === true)) {
                                ...
                                //  AUDITORÍA: REGISTRO EXITOSO CON CENTROS DE COSTO
                                ...
                                foreach ($materiales as $mat) {
                                    ...
                                    //  INCLUIR CENTROS DE COSTO EN AUDITORÍA
                                    $cc_count = count($mat['centros_costo']);
                                    $productos_detalle[] = "$desc_corta (Cant: {$mat['cantidad']}, CCs: $cc_count)";
                                }
                                $desc_productos = implode(' | ', $productos_detalle);

                                $descripcion = "ID: $id_salida | Origen: $almacen_origen_nombre | Destino: $almacen_destino_nombre | Productos: $desc_productos";
                                ...
                                if ($id_salida > 0 && isset($_FILES['documento']) && !empty($_FILES['documento']['name'][0])) {
                                ...
                                // Finalizar pedido si viene de uno
                                ...
                                GrabarAuditoria($id, $usuario_sesion, 'ERROR AL REGISTRAR', 'SALIDAS', "Error del sistema");
