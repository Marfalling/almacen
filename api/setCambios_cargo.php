...
$nom = $_POST["nom_cargo"] ?? null;
$act = $_POST["act_cargo"] ?? 1;
...
$sqlSelect = "SELECT updated_at FROM cargo WHERE id_cargo=?";
...
if ($resultSel->num_rows > 0) {
    ...
    if (strtotime($updated_at_remoto) > strtotime($updated_at_local)) {
        ...
        $sqlUpdate = "UPDATE cargo 
                      SET nom_cargo=?, act_cargo=?, updated_at=? 
                      WHERE id_cargo=?";
...
$sqlInsert = "INSERT INTO cargo (id_cargo, nom_cargo, act_cargo, updated_at)
              VALUES (?, ?, ?, ?)";
